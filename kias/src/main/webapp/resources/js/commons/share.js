...
Share.openTab = function(node, url, parentTab, iconCls) {
	var id = node.data.id, title = node.data.text,
	// 设置在哪个元素打开
	tabPanel = parentTab === undefined ? main.rightPanel : parentTab; //css = iconCls === undefined ? node.attributes.iconCls : iconCls;
	// 参数验证
			...
			listeners : {
				activate : function(Panel) {
					...
					if (inPanel) {
						...
						inPanel.setHeight(main.tabPanel.getInnerHeight() - 1);
